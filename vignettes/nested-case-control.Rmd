...
The case-cohort design works in the following way: For each observed failure-tim (case), sample $m-1$ non-failures (controls) without replacement. Nested case-control is similar, but with the difference that the sampling is done, only among those at risk at the cases failure-time. 
