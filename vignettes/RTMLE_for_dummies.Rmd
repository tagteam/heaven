...
```{results='asis'}
    ...
    x <- target(x,name = "A",strategy = "additive",estimator = "tmle",protocols = c("A","not A"))
```
...
```{results='asis'}
    x <- protocol(x,name="A1",
                         intervention = data.frame("A" = factor(c("1","1","0","0"),levels = c("0","1"))))
    x <- target(x,name="A1", strategy="additive",estimator="tmle",protocols = c("A1","not A"))
```
...
```
The next step is **model_formula** - needs explanation
```{results='asis'}
x <- model_formula(x)
```
The calculations are inititated with **run_rtmle**. A very simplisticv version is here:
```{results='asis'}
...
## NEW
x <- rtmle_init(intervals=3,name_id='ID',name_time='period',name_outcome='Y',
                name_competing='CO',name_censoring='CE',
                censored_levels=c('1','0'),censored_label="1")
x <- add_wide_data(x,
               outcome_data = outcome_dt,
               timevar_data=treatment_dt
...
x <- protocol(x,name = "A",treatment_variables = "A",intervention = 1)
x <- protocol(x,name = "not A",treatment_variables = "A",intervention = 0)
x <- target(x,name = "Risk",strategy = "additive",estimator = "tmle",protocols = c("A","not A"))
x <- model_formula(x)
x <- run_rtmle(x,learner = "learn_glm",time_horizon = 1:3)
summary(x)
...
Only necessary new objects are included
```{r echo=TRUE}
target(x) <- list(name = "Complex learner",strategy = "additive",estimator = "tmle",protocols = c("A","not A"))
x <- run_rtmle(x,
               targets="Complex learner",
               time_horizon = 2,
               refit = TRUE,
               learner = list("learn_ranger_50" = list(num.trees = 20,learner_fun = "learn_ranger"),
                              "learn_glmnet"),folds = 10)
...
x <- target(x,name = "Active comparitor",strategy = "additive",estimator = "tmle",protocols = c("Always_A_never_B","Always_B_never_A"))
x <- model_formula(x)
