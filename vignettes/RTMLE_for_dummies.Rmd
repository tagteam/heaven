...
A longitudinal study is considered where participants at time zero receive one of two treatments (A=0/1). Subject selection may rely on starting a treatment of interest versus a selected control treatment or it may rely on comparing subjects starting a selected treatment with controls selected by a variety of mechanisms which could be matching.  Note that if matching users to non-users what is calculated is average treatment effect of the treated rather than average treatment effect.
...
sim <- trialSimulation(n = 1000, seed = 123)
