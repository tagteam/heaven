...
extractIcd <- function(dat,disease=NULL,inclusions=NULL,exclusions=NULL,p.in=NULL,p.out=NULL,mult=FALSE,
  ...
  disease_defs <- list(af=c("DI48","42793", "42794"),rheumvalve=c(as.character(c(39400, 39401, 39402, 
                     ...
                     stroketci=c(as.character(c(43309, 43399, 43409, 43499, 43600, 43601, 43609, 43690, 43699, 43700,
                                               43701, 43708, 43709, 43790, 43791, 43798, 43799, 43809, 43899)),
                     ...
                     perart=c('DI70',as.character(c(44009, 44019, 44020, 44021, 44028, 44029, 44030, 44039, 44099))),
  ...
  if(!is.null(index)&!is.null(index.int)){
    out <- out[index>=inddto&(index-index.interval)<=inddto]
  }
  
  # Find diagnoses after an index date (outcome)
  if(!is.null(index)&outcome==TRUE){
    out <- out[index<inddto]
  }
  ...
  return(list(data=out,diagnoses=diags_for_extraction,unique.icd=icdcodes$diag))
}
