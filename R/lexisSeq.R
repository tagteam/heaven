...
#' 
#' @description 
#' lexisSeq is used to split by a sequence of dates - typical use is a sequence 
#' of calender dates (time periods) or a sequence of times after birth to split 
#' by age in a time dependent manner. Splitting times is provided by 
#' "splitvector" and "varname".  
#' 
#' If all subjects are to be split by the same times "varname" should be "NULL".  
#' If "varname" is a number it is added to the vector.  A typical use for this 
#' adding is to split by age. The vector identifies a series of times after 
#' birth and "varname" is the date of birth.
#' 
#' The vector can be provided as a series of fixed times with format="vector".  
#' It can also be provided as from-to-by, three values with format="seq".  
#' In that case a vector is defined with all values between "from" and "to" by 
#' each interval of "by".
#' 
#' The variable with default name "value" takes a value of zero to the left of 
#' the first interval and increased by 1 as each element in the vector is 
#' passed.
#' 
...
#' @param indat - base data with id, start, end, event and other data - 
#' possibly already split
#' @param invars - vector of colum names for id/entry/exit/event - 
#' in that order, example: c("id","start","end","event")
#' @param varname - name of variable to be added to vector
#' @param splitvector - A vector of calender times (integer). Splitvector can 
#' be a sequence of fixed times with format="vector" or generate a sequence of 
...
#' The function extracts thos variables necessary for splitting, splits by the 
#' provided vector and finally merges other variable onto the final result.
