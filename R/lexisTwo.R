...
#' 
#' @description 
#' lexixTwo is a specialised version of lexis splitting.  While preparing data 
#' for time dependent analyses with e.g. Cox org Poisson regression records need 
#' to be "split" at selected times.  For practical use there are often  multiple 
#' conditions such as comorbidities that will cause either a single split or no 
#' splitting.  This function is designed to receive base data with 
#' start/end/event and another set of data where patient id is accompanied with 
#' one column per condition with these columns containing the time where split 
#' should occur.
#' @usage
#' lexisTwo(indat,splitdat,invars,splitvars)
#' @author Christian Torp-Pedersen
#' @param indat - base data with id, start, end, event and other data - 
#' possibly already split by other functions
...
#' event after last interval.
#' 
#' The other table contains a single record for each participant and columns to 
#' identify dates to split by.  After splitting all intervals preceding the date 
#' will have a variable identified by each column with the value "0". After the 
#' date the value i "1".
#' In the example the columns are dat1-date4 - but it is most useful to provide 
#' names that identify the condition which changes by the date rather than a 
#' names which indicates a date.
...
#' split <- data.table (id=c("A","B","C","D"),date1=c(0,NA,150,300),
#'    date2=c(25,75,175,325),date3=c(30,30,175,325), date4=c(0,1,0,1))
