                           ...
                           condition.name = "X") 
{
  ...
  for (i in 1:length(conditions)) {
    Ex <- FALSE # No exclusion list default
    cc <- conditions[[i]]
    name <- names(conditions)[[i]]
    regexp <- switch(match, exact    = paste0("^", cc,"$", collapse = "|"), 
                     contains = paste0(cc,collapse = "|"), 
    ...
    if (!is.null(exclusions) & name %in% names(exclusions)){
      ...
      regexpEx <- switch(match, 
                         ...
                         start    = paste0("^",ee, collapse = "|"),
                         end      = paste0("$",ee,collapse="|"))
      Ex <- TRUE 
    }
    if (length(vars) == 1) {
      found <- grepl(regexp, data[[vars]])
      if (Ex){
        foundEx <- grepl(regexpEx,data[[vars]])
        found <- found & !foundEx  
      }
    }
    else {
      ...
      if (Ex){
        foundEx <- rep(0L, NROW(data))
        for (v in vars) {
          foundEx <- foundEx | grepl(regexpEx, data[[v]])
        }  
      }
      if (Ex) found <- found & !foundEx
    }  
    if (length(out) == 0) {
      out <- data[found]
      out[, `:=`(cond, name)]
