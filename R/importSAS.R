...
##'
##' # Also, it is often a good idea to read a limited amount of data
##' df101 <- importSAS(filename="X:/Data/Rawdata_Hurtig/704791/diag_indl",obs=101)
##' # and to examine the result
##' str(df101)
##' df101
##' # If this is fine, then one can import the whole file as follows.
##' # df <- importSAS(filename="X:/Data/Rawdata_Hurtig/704791/diag_indl",save.tmp=TRUE)
...
##' df2
##'
##' # Save all the output and the csv file in a specific file named "savedfile.csv" in the directory "dir".
##' #df <- importSAS(filename="X:/Data/Rawdata_Hurtig/704791/diag_indl",obs=101,savefile="savedfile.csv",
##' #                save.tmp=TRUE,overwrite=FALSE)
##'
...
importSAS <- function(filename,
    ...
    try({
        ...
        if (length(keep)>0) is.date <- is.date[var.names %in% keep]
        if (length(drop)>0) is.date <- is.date[!(var.names %in% drop)]
        format.statement <- if (sum(is.date)==0) "" else paste("format ",paste(var.names[is.date],collapse=" ")," yymmdd10.;")
        # }}}
        # {{{ Write SAS file, export filterfile if present
        if(length(filter)>0){
            ...
            cat("proc sort data=csv_import; \nby ",
                ...
                append=TRUE)
            ### Merge the files
            cat("data df; \nmerge csv_import(IN=a) fulldata(IN=b); \nby ",
                paste(filter.names, collapse=" "),
                ";\nif a AND b;\nrun;\n",
                sep = "",
                file=tmp.SASfile,
        ...
        if(content==TRUE){
            ...
            if(!(prod(keep.check)*prod(drop.check)*prod(filter.check))) {
                ...
                df <- data.table::fread(file = outfile, header = TRUE)
                if (sum(is.date)>0){
