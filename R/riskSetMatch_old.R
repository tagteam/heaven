...
#'   reuseCases=FALSE,reuseControls=FALSE,caseIndex=NULL,controlIndex=NULL,
#'   NoIndex=FALSE,cores=1) 
...
#' out <- riskSetMatch_old("ptid","case",c("byear","sex"),dat,2,NoIndex=TRUE)
#' out[]
#' # Risk set matching without reusing cases/controls - Some cases have no controls
#' out2 <- riskSetMatch_old("ptid","case",c("byear","sex"),
#'                       dat,2,caseIndex="caseIndex",
#'                      controlIndex="controlIndex")
#' out2[]   
#' # Risk set matching with reuse of cases (control prior to case) and reuse of 
#' # controls - more cases get controls
#' out3 <- riskSetMatch_old("ptid","case",c("byear","sex"),dat,2,caseIndex=
#'   "caseIndex",controlIndex="controlIndex"
#'   ,reuseCases=TRUE,reuseControls=TRUE)
#' out3[]   
#' # Same with 2 cores
#' out4 <- riskSetMatch_old("ptid","case",c("byear","sex"),dat,2,caseIndex=
#'   "caseIndex",controlIndex="controlIndex"
#'   ,reuseCases=TRUE,reuseControls=TRUE,cores=2)  
#' out4[]           
riskSetMatch_old <- function(ptid     # Unique patient identifier
    ...
    ## options(warn=-1)
    pnrnum=.ptid=cterms=.SD=.event=.cterms=label=random=.controlIndex=.caseIndex=.N=NULL
    ...
    if (cores<2){
        ...
        selected.controls <- do.call("rbind",lapply(split.alldata,function(controls){
            ...
            Output <- .Call('_heaven_Matcherold',PACKAGE = 'heaven',Ncontrols,Tcontrols,Ncases,NreuseControls,controlIndex,caseIndex,controls[,pnrnum],cases[,pnrnum],noindex)
            setDT(Output)
    ...
    FINAL <- rbind(cases[,data.table::data.table(pnrnum,caseid,.event)],selected.controls[,data.table::data.table(pnrnum,caseid,.event)])
    setkey(FINAL)
    #output
    datt[,(event):=NULL]
    ...
    if (!NoIndex) {
      FINAL[,.newcaseindex:=caseIndex[.N],by=caseid]
