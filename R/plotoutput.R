...
plot.dppout <- function(x, drug=NULL, id=NULL, idmax=9, trace=FALSE, fix_x=FALSE, normalize=FALSE,...) {
    ...
    drugout <- data.frame(data.table::rbindlist(lapply(j, function(j) {
        dat <- x[[j]]
    ...
    natc <- length(j)
    col <- colorRampPalette(c("darkblue", "darkgreen"))(natc)
    ...
    if (TRUE) {
        ...
        if (fix_x) {
            ...
            gout <- gout + scale_x_date(limits = period, date_labels = "%y/%m")
        } else 
            ## gout <- gout + scale_x_date(labels = date_format("%b %Y"))
            gout <- gout + scale_x_date(date_labels = "%b %Y")
        ...
        B2 <- data.table::rbindlist(lapply(1:(dim(drugout)[1]), function(i) {
