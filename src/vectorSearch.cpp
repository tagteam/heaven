    ...
    for(int j=0; j<ni; j++){ // Loop through inclusion criteria blocks
      ...
      for(int jj=0; jj<ilength;jj++){ // Loop through individual inclusion criteria
        ...
        case (1):  
          ...
          break;
        case (2):
          if (searchCols[i].size()>=conditions[innum].size() && searchCols[i].rfind(conditions[innum])==searchCols[i].size()-conditions[innum].size()) include=1;
          break;
        case (3):
          ...
          for(int k=0; k<ne; k++){ // loop though exclusion blocks
            ...
            for(int kk=0; kk<elength; kk++){ // Loop though individual exclusion criteria
              ...
              case(1):  
                ...
                break;
              case(2):  
                if (searchCols[i].size()>=exclusions[exnum].size() && searchCols[i].rfind(exclusions[exnum])==searchCols[i].size()-exclusions[exnum].size()) exclude=1;
                break;
              case(3):  
