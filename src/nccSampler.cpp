...
// [[Rcpp::export]]
DataFrame nccSamplingCpp(arma::vec pnr, arma::vec time, arma::vec status, arma::vec Tstart, 
			 arma::vec exposureWindow, int Ncontrols=10){
  ...
  for(int i=0;i<n;++i){                               
    ...
    if(ncont>0){                                                  // Do stuff if risk set is not empty
      ...
      PNR.elem(pos) = pnr.elem(find(Case));                       // PNR for cases
