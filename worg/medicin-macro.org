...
#+BEGIN_SRC latex :export results :eval t
        ...
        \\ & \qquad + \, u_{k-1}\, 1\{\tilde{\str}_{k-1}=\tilde{\str}_{k}\} \,\,
 \text{round} \left(\frac{1}{\tilde{s}^{\min}_k} \, 
   \frac{
     \sum_{\ell \in I^{(1)}_k} \,D_\ell}{ \sum_{\ell \in I^{(1)}_k} \,
      H_\ell} 
  \right) \, \tilde{s}^{\min}_k
 \\ & \qquad +
        \,\Big( u_{k} \,(1-u_{k-1}) + u_{k-1} 1\{\tilde{\str}_{k-1}\ne \tilde{\str}_{k}\} \Big)\,
\Big( m^{1}_k \, \tilde{s}^{\max}_k + m^{0}_k \, \tilde{s}^{\min}_k + 
(1- m^{1}_k)(1- m^{0}_k) \tilde{s}^*_k\Big).
        \end{align*}
        Here, \(\tilde{\str}_k\) is the nearest of the predefined pill strengths not
        exceeding the average strength computed for a period \(T_k\), that
        is,
      \begin{align*}
        \tilde{\str}_k = \underset{ s \in \lbrace S_1,\ldots, S_J\rbrace}{\max} 
    \left\lbrace s \le  \frac{1}{c_{k}}  \sum_{j=1}^J \one \lbrace n_{ j}>0\rbrace\, S_{j} \right\rbrace, 
      \end{align*}
and we let  \(\tilde{s}^{\min}_k\), \(\tilde{s}^{\text{max}}_k\) and \(\tilde{s}^*_k\) denote the to minimal, maximal and typical dose corresponding to \(\tilde{\str}_k\). 
...
together with
\begin{align*}
m^{1}_k = \one \left\lbrace 
   \frac{
     \sum_{\ell \in I^{(2)}_k} \,D_\ell}{ \sum_{\ell \in I^{(2)}_k} \,
      H_\ell} > \tilde{s}^{\max}
 \right\rbrace, \qquad
m^{0}_k = \one \left\lbrace 
   \frac{
     \sum_{\ell \in I^{(2)}_k} \,D_\ell}{ \sum_{\ell \in I^{(2)}_k} \,
      H_\ell} < \tilde{s}^{\min}
 \right\rbrace
